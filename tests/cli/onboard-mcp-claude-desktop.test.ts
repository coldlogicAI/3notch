...
describe('notch onboard --mcp claude-desktop', () => {
  it('updates injected Claude Desktop config, preserves existing servers, and writes a backup', async () => {
    await withTempProject({}, async (project) => {
      ...
      const result = await runCli(['onboard', '--yes', '--mcp', 'claude-desktop'], {
        cwd: project.path,
      ...
      expect(result.stdout).toContain('Configured claude-desktop MCP server "3notch"');
